...
$EditarCota = (isset($_POST['EditarCota'])) ? $_POST['EditarCota'] : 0;
...
$VerificarCaracterSenha = VerificarCaracter($EditarSenha);
	...
	if($VerificarInfoPre[0] == "S"){
		$SQLCota = "SELECT Cota FROM rev WHERE usuario = :usuario";
		$SQLCota = $painel_user->prepare($SQLCota);
		$SQLCota->bindParam(':usuario', $EditarUsuario, PDO::PARAM_STR);
		$SQLCota->execute();
		$LnCota = $SQLCota->fetch();
		$CotaRev = empty($LnCota['Cota']) ? 0 : $LnCota['Cota'];
		
		$EditarPrePago = $VerificarInfoPre[0];
		$EditarTempo = $VerificarInfoPre[2];
		$NovaCota = ($VerificarInfoPre[1] + $CotaRev) - $EditarCota;
	}
	...
	elseif(!in_array($EditarUsuario, $ArvoreAdminOnline)) {
	...
	if( ($EnviarEmail == "on") && ($EnviarEmailSend == 1) && ($EnviarSMS == "on") && ($EnviarSMSSend == 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsfeueeus'], "success", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarEmail == "on") && ($EnviarEmailSend != 1) && ($EnviarSMS == "on") && ($EnviarSMSSend != 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsoueaeueeus'], "warning", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarEmail == "on") && ($EnviarEmailSend == 1) && ($EnviarSMS == "on") && ($EnviarSMSSend != 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcseecsoueaeos'], "warning", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarEmail == "on") && ($EnviarEmailSend != 1) && ($EnviarSMS == "on") && ($EnviarSMSSend == 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsoueaeoesecs'], "warning", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarEmail == "on") && ($EnviarEmailSend == 1) ){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsfeue'], "success", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarSMS == "on") && ($EnviarSMSSend == 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsfeus'], "success", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarEmail == "on") && ($EnviarEmailSend != 1) ){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsoueaeoe'], "warning", "index.php?p=revendedor");
		exit;
	}
	elseif( ($EnviarSMS == "on") && ($EnviarSMSSend != 1)){
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcsoueaeos'], "warning", "index.php?p=revendedor");
		exit;
	}
	...
	if(empty($SQL)){
		...
		echo MensagemAlerta($_TRA['sucesso'], $_TRA['alcs'], "success", "index.php?p=revendedor");
	}
